...
The recommended demonstration flow is designed around a concise end-to-end recovery story.
...
## Step 2 — Select a Failed Transaction

Open the Revenue at Risk Explorer.

Select a high-priority case.
...
Use the PayNexa AI Copilot to explain:

* Why the transaction is at risk
...
Open the Recovery Action Center.
...
Trigger a simulated recovery action such as:

**Smart Retry**

or another allowed recovery workflow.

---

## Step 6 — Recovery Outcome

Show the resulting transaction state:
...
Open a pending offline/cash verification case.

Demonstrate:

```text
Review → Verify / Reject → Outcome → Audit
```
...
> **Failed Payment → AI Analysis → Recovery Recommendation → Policy Control → Recovery → Outcome → Audit → Measured Revenue**

---

# 🔐 Security & Responsible AI

PayNexa follows responsible automation principles.

## API Key Protection

API keys should never be hardcoded into application source code or committed to GitHub.

Use environment variables or secure server-side secret management.
